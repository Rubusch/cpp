...
void solve_equation(double val_a, double val_b, double val_c, double val_d)
{
  std::cout << "solve equation:\n"
            << val_a << "*x*x " << std::setiosflags(std::ios::showpos) << val_b
            << "*x " << val_c << " = " << std::resetiosflags(std::ios::showpos)
            << val_d;
  ...
  if (cmx_x1 != cmx_x2) {
    std::cout << "\nresults: x1 = " << cmx_x1
              << " [real, imaginative] and x2 = " << cmx_x2
              << " [real, imaginative]" << std::endl;
  } else {
    std::cout << "\nresult: x = " << cmx_x1 << " [real, imaginative]"
              << std::endl;
  }
  std::cout << std::endl;
}
