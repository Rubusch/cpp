...
int main()
{
  ...
  const int *ptr = find_first_of(arr_b, arr_b + 5, arr_a, arr_a + 4);
  if (ptr != arr_b + 5) {
    cout << *ptr
         << " is the first element which is contained in arr_b and in arr_a.\n";
  }
  cout << endl;

  cout << "find_first_of() - second example\n";
  ptr =
      find_first_of(arr_b, arr_b + 5, arr_b, arr_b + 4, not_equal_to< int >());
  if (ptr != arr_b + 5) {
    cout << *ptr
         << " is the first element contained in arr_b but NOT in arr_a.\n";
  }
  cout << endl;
