...
template < class T >
class SumUp : public std::unary_function< T, void >
{
private:
  T sum_;

public:
  explicit SumUp(const T &sum = T()) : sum_(sum) {}
...
/*
  a or b
//*/
void a_or_b(int idx) { std::cout << ((idx % 2 == 0) ? 'a' : 'b'); }
...
int main()
{
  ...
  cout << "...which is actually the same like the following implementation "
          "(doing the same):\n";
