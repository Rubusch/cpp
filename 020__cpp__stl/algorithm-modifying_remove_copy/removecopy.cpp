...
int main()
{
  ...
  int arr_b[8]; fill(arr_b, arr_b+8, 0);
  cout << "arr_a \t";
  display(arr_a, 8);
  cout << endl;
  ...
  cout << "ptr shows to the end of the range (last element that is not the omitted), the whole range looks like\nrange: \t";
  display(ptr-5, 8); // ptr points to the end of the new range, -5 returns the "full" new copy
  cout << endl;

  cout << "remove_copy_if() - copy only the even numbers of arr_b to ptr:\n";
  remove_copy_if(arr_b, ptr, ostream_iterator< int >(cout, " "), bind2nd(modulus< int >(), 2));
  cout << endl << endl;
  cout << "arr_a \t";
  display(arr_a, 8);
  cout << "arr_b \t";
