...
/*
  the actual factory method, implemented as "static" function in the protocol class
//*/
template<class T>
...
public:
  // ctor
  RealMonster(const T& name, const Size_type<T>& size, const Color_type<T>& color);
...
template<class T>
RealMonster<T>::RealMonster(const T& name, const Size_type<T>& size, const Color_type<T>& color)
