...
void Plotter::initPlotSettings()
{
  ...
  //*/
  // size of the widget
  setMinimumWidth(((settings->numXTicks + 1) * Margin) + (2 * Margin) -
                  settings->numXTicks); // FIXME
  setMinimumHeight(((settings->numYTicks + 1) * Margin) + (2 + Margin) -
...
void Plotter::drawPoint(QPainter *painter, QRect rect, const double xCoord,
                        const double yCoord)
{
  static const QColor curveColor = Qt::red;
  if (!rect.isValid())
