...
/*
  ...
  block of memory is first broken down into Foobar-sized chunks, and these chunks are then placed

  on the free list. When clients call Foobar::operator new, chunks are removed from the free list,
  ...
  memory pool. Call it semantic gymnastics if you must, but there is an important difference between a
  memory leak and a memory pool. A memory leak may grow indefinitely, even if clients are wellbehaved,
  but a memory pool never grows larger than the maximum amount of memory requested by
  its clients."
...
//*/
Foobar *Foobar::headOfFreeList; // headOfFreeList doesn't need explicitely to be set to NULL
const int Foobar::BLOCK_SIZE = 512;

void* Foobar::operator new(size_t size)
{
  std::cerr << "I am the overloaded Foobar::operator new()" << std::endl; // some comment
  ...
  if(ptr){
    ...
    Foobar *newBlock = static_cast<Foobar*>(::operator new(BLOCK_SIZE * sizeof(Foobar)));

    // create new freelist, but exclude 0. element (head), the 0. element will be returned
...
{
  std::cerr << "I am the overloaded Foobar::operator delete()" << std::endl; // some comment

  // check
  if(NULL == rawDeadObj) return;
