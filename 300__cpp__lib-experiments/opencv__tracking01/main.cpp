...
IplImage *GetThresholdedImage(IplImage *frame)
{
  ...
  //       cvInRangeS( imgHSV, cvScalar( 38, 70, 70 ), cvScalar( 75, 256, 256 ),
  //       imgThresh );
...
int main(int argc, char *argv[])
{
  ...
  while (true) {
    ...
    /* TODO failing
                // compute the moments to estimate the position of the target
    object CvMoments *moments = NULL; if( NULL == (moments = (CvMoments*)
    malloc( sizeof( CvMoments ) )) ){ qDebug( "ERROR: allocation failed\n" );
                        break;
                }
                ...
                if( xlast > 0
                  ...
                  cvLine( imgScribble, cvPoint( xpos, ypos ), cvPoint( xlast,
    ylast ), cvScalar( 0, 255, 255 ), 5 );
                }
