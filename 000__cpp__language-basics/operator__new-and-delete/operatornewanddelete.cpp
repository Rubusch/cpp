...
/*
  ...
  Adhere the convention when writing "operator new()" and "operator delete()".
(8/Meyers)

TODO review compilation warning
g++ -c -g -Wall  opnewdelete.cpp
opnewdelete.cpp: In static member function ‘static void* Foobar::operator
new(size_t)’: opnewdelete.cpp:55:15: warning: catching polymorphic type ‘class
std::bad_alloc’ by value [-Wcatch-value=] }catch(std::bad_alloc){
...
void *Foobar::operator new(size_t size)
{
  ...
  void *memory;
  //  try{ // keep it 'exception neutral', i.e. let exception handling happen
  //  totally on client site (sh. Exceptional C++, Herb Sutter)
  memory = ::operator new(size);
  //  }catch(std::bad_alloc &e){
  //    std::cerr << "Uh-oh.. something went wrong with the \"operator new()\""
  //    << std::endl; // some comment
  //    // MISSING: in case of a modified new-handler reset here to the global
  //    new-handler throw;
  //  }
...
void Foobar::operator delete(void *rawMemory, size_t size)
{
  ...
  if (size != sizeof(Foobar)) {
    std::cerr << "global \"operator delete()\" was used"
              << std::endl; // some comment
