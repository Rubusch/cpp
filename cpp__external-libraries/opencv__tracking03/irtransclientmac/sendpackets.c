	...
	char *argv[];
{
	...
	if (argc!=6)
	{
		fprintf(stderr,"Exacty 5 parameters are required. Received %d parameters.\n",argc-1);
		fprintf(stderr,"    %s <id> <pos_x> <pos_y> <target_x> <target_y>",argv[0]);
		return 1;
	}
	...
	char number[3] = " 0";
	int ret=0;
	sprintf(command, "../irtransclientmac/irclient64 %s -sequence rc5 volup",address);
	// ret = system(command);
