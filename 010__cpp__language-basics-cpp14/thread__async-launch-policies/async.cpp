/*
  ...
  The 'std::async' creates a task, that may be parallelized by the
  compiler to asynchronous threads.

  It may start right away (std::launch::async) or its start may be
  deferred (std::launch::deferred), triggered at a call to 'wait()' or
  get results.
  ...
  pool) and returns a std::future that will eventually hold the result
  of that function call.

  The implementation may extend the behavior of the first overload of
  std::async by enabling additional (implementation-defined) bits in
