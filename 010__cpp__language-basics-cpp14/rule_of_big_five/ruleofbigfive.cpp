/*
 ...
 if move is applied to e.g. 'const string' or any other const, move will rather
 copy the content than move anything! (Item 23, Effective Modern C++, Meyers,
 2015)
...
public:
  ...
  Box &operator=(Box &rhs) noexcept
  {
    ...
    if (this !=
        &rhs) { // can be left out (optimization), then will need exception
                // handling (Exceptional C++, Herb Sutter, 2000)
      if (pItem)
        delete pItem;                    // avoid memory leak
  ...
  const string &getItem() { return *pItem; }

  void setItem(const string *item) { pItem = item; }
};
...
int main(void)
{
  ...
  // the box itself
  Box box(pStr);
  cout << "addr of 'box::pItem': \t\t\t" << &box.getItem()
       << " <- same as pStr, since there allocated" << endl;
  cout << endl;

  // call copy assignment constructor
  Box copyBox(box);
  cout << "addr of 'cp-box::pItem': \t\t" << &copyBox.getItem()
       << " <-- deep copy, differs" << endl;
  cout << endl;

  // call copy assignment operator
  Box backupBox;
  backupBox = box;
  cout << "addr of 'asn-box::pItem': \t\t" << &backupBox.getItem()
       << " <-- deep copy differs" << endl;
  cout << endl;

  // call copy move constructor
  Box movedBox = std::move(box);
  cout << "addr of 'move-ctr-box::pItem': \t\t" << &movedBox.getItem()
       << " <-- SAME as box::pItem!!!" << endl;
  cout << "addr of 'box::pItem': \t\t\t" << &box.getItem() << " <-- now is null"
       << endl;
  cout << endl;
  ...
  cout << "addr of 'moving-box::pItem': \t\t" << &movingBox.getItem()
       << " <-- SAME as move-ctr-box::pItem!!!" << endl;
  cout << "addr of 'movedbox::pItem': \t\t" << &movedBox.getItem()
       << " <-- now is null" << endl;
  cout << endl;
