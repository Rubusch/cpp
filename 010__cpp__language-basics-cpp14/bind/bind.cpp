...
/*
  ...
  demostrates the usage of bind (C++11), argument reordering and
pass-by-reference then bind examples then are shown as lambda implementation
...
int anotherFunc(int n1) { return n1; }


struct SomeClass {
  int data = 10;

  void print_sum(int val1, int val2) { cout << val1 + val2 << '\n'; }
};
...
int main()
{
  ...
  cout << "bind arguments to printer function, and create a parametrized "
          "printer object"
       << endl
  ...
  auto printerLambda = [cnum = std::cref(num), num](int n1, int n2,
                                                    int ignored) {
    printer(n2, 42, n1, cnum, num);
  };
  num = 10;
  ...
  cout << "nested bind in bind, the bind subexpression shares the placeholders!"
       << endl
       << "bind:\t\t[_3]\t[bind]\t[_3]\t4\t5\t// [_3] via nested bind() to "
          "anotherFunction"
       << endl;
  auto anotherPrinterObject =
      std::bind(printer, _3, std::bind(anotherFunc, _3), _3, 4, 5);
  auto anotherPrinterLambda = [](int n1, int n2, int n3) {
    printer(n3, anotherFunc(n3), n3, 4, 5);
  };
  ...
  cout << "use case: bind a random number generator to a function object"
       << endl
       << "bind:\t\tuid\tdre" << endl;
  std::default_random_engine dre;
  std::uniform_int_distribution<> uid(0, 10);
  auto randomDistributionObject =
      std::bind(uid, dre); // a copy of dre is stored in rnd
  ...
  cout << "bind a pointer to a data member as static instruction, then passing "
          "the specific object"
       << endl
       << "bind:\t\t"
       << "[_1]" << endl;
  ...
  cout << "smart pointers can be used to call members of the referenced "
          "objects, too"
       << endl;
  cout << "shared ptr: " << dataMemberObject(make_shared< SomeClass >(foo))
       << endl;
  cout << "unique ptr: " << dataMemberObject(make_unique< SomeClass >(foo))
       << endl;

  // access through shared pointers will be less convenient (better solution
  // possible? ) since shared_ptr / unique_ptr are more type safe than the vague
  // 'std::bind' sometimes this may be an advantage in convenience of
  // 'std::bind' though
  cout << "shared ptr (lambda): "
       << dataMemberLambda(*make_shared< SomeClass >(foo).get()) << endl;
  cout << "unique ptr (lambda): "
       << dataMemberLambda(*make_unique< SomeClass >(foo).get()) << endl;

  // explicit shared_ptr variants of the lambda might look as follows
  auto shPtrDataMemberLambda = [](shared_ptr< SomeClass > psc) {
    return psc->data;
  };
  cout << "special shared ptr lambda: "
       << shPtrDataMemberLambda(make_shared< SomeClass >(foo)) << endl;
