...
public:
  Fruit(const string *item) { pItem = item; }
  ...
  // print out class
  friend ostream &operator<<(ostream &out, Fruit &fruit);
};
...
ostream &operator<<(ostream &out, Fruit &fruit)
{
  if (fruit.pItem)
    return out
           << *fruit.pItem; // segfaults: when copy assignment ctor is missing
  return out << "";
}
...
int main(void)
{
  vector< Fruit > box;
  box.push_back(
      Fruit(new string("Lemon"))); // double free: when copy move ctor missing
  ...
  box.erase(remove_if(box.begin(), box.end(),
                      [x](Fruit fruit) {
                        cout << "CALLED: lambda for '" << *fruit.getItem()
                             << "' " << endl;
