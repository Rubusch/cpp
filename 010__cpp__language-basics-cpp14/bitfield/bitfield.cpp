...
int main(void)
{
  ...
  cout << "another derived bitfield adbf created (only one initialization)" << endl;
  DerivedBitField adbf = DerivedBitField();
  adbf.bits = 7;
  cout << "adbf.bits: " << adbf.bits << ", adbf.another_bits: " << adbf.another_bits << endl;
  cout << endl;

// note:
//  printf("bf: 0x%lX", (unsigned long)bf); // ERROR, not possible to read out bits directly
