...
public:
  Binder(const Operation &op, const T &arg) : op_(op), item_(arg) {}
...
/*
  step 3/3: a function to connect the two functors.. in STL this is used as
"trait"
...
//*/
template < class Operation, class T >
inline Binder< Operation, T > bindit(const Operation &op, const T &item)
{
...
int main()
{
  ...
  vector< int >::iterator iter =
      find_if(vec.begin(), vec.end(), bindit(EqualsTo< int >(), 50));
  cout << endl;

  cout << "\'50\' was contained? -> " << ((iter != vec.end()) ? "Yes!" : "No!")
       << endl;
