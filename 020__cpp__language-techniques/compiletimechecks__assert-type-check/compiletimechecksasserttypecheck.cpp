...
/*
  ...
  "Suppose, for instance, you are developing a function for safe casting. You
want to cast from one type to another, while making sure that information is
preserved; larger types must not be cast to smaller types."

  The original code snippet is thought for a situation using the very "libaral"
  reinterpret_cast - this didn't work out for me working with gcc 4.3.2,
reinterpret_cast of a bigger type into a smaller one is not possible (allowed in
standard?).

  I could enter a similar situation, though, using the static_cast or typedef.
Since I'd like to stay in C++, though, I use here the static_cast() for the
example.
...
int main()
{
  ...
  /*
    INTERESTING:
    The template is defined by two template types, but finally only needs to
  have the destination type. It doesn't need the FULL template section!!
  //*/
