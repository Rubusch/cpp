// errormessage.cpp
/*
  Techniques, item 3: Provoking Compile-time Errors and Providing a Error
Message

  Manipulating the Error Messages:
  "The problem with" the approach getting compile-time errors "is that the error
message you receive is not terribly informative. 'Cannot create array of size
zero' does not suggest 'Type char is too narrow to hold a pointer.' It is very
hard to provide customized error messages portably. Error messages have no rules
that they must obey; it's all up to the compiler."
...
template < bool >
struct CAUTION_type_too_small_to_cast_to;
template <>
struct CAUTION_type_too_small_to_cast_to< true > {
};
