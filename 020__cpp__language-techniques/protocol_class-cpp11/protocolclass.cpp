...
/*
  ...
  A protocol is an abstract class that contains the "protocol" of the members to
implement, each "value" is abstracted by a function to gain some kind of type
independence.

  A good explanation of a protocol class is contained in Effective C++ / Item 34
/ Meyers.
...
public:
  Size_type(T size) : data(size) {}

  T getSize() const { return data; }
};
...
public:
  Color_type(T color) : data(color) {}

  T getColor() const { return data; }
};
...
public:
  Species_type(T species) : data(species) {}

  T getSpecies() const { return data; }
};
...
/*
  protocol class / interface (abstract)
//*/
template < class T >
class AnimalProtocol
{
public:
  ...
  // just for generating some output
  friend ostream &operator<<< T >(ostream &out, AnimalProtocol< T > &animal);

  // inner class (exception type)
  class BadAnimalException : public exception
  {
  public:
    BadAnimalException() = default;
    ~BadAnimalException() throw() = default;
  ...
  static AnimalProtocol< T > *
  animalFactoryMethod(const Species_type< T > &species,
                      const Size_type< T > &size,
                      const Color_type< T > &color) noexcept(false);
};
...
template < class T >
inline ostream &operator<<(ostream &out, AnimalProtocol< T > &animal)
{
  return out << "species\t: " << animal.getSpecies()
             << "\nsize\t: " << animal.getSize()
             << "\ncolor\t: " << animal.getColor() << endl;
}
...
public:
  RealAnimal(const Species_type< T > &species, const Size_type< T > &size,
...
template < class T >
RealAnimal< T >::RealAnimal(const Species_type< T > &species,
                            const Size_type< T > &size,
                            const Color_type< T > &color)
...
//*/
template < class T >
AnimalProtocol< T > *AnimalProtocol< T >::animalFactoryMethod(
    const Species_type< T > &species, const Size_type< T > &size,
    const Color_type< T > &color) noexcept(false)
{
...
int main()
{
  ...
  cout << "Init..\n" << endl;
  AnimalProtocol< string > *pAnimal = nullptr;
  ...
  try {
    pAnimal =
        AnimalProtocol< string >::animalFactoryMethod(species, size, color);
