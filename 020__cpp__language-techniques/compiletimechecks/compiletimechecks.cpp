...
/*
  ...
  "The idea" here "is to pass the compiler a language construct that is legal
for a nonzero expression and illegal for an expression that evaluates to zero.
This way, if you pass an expression that evaluates to zero, the compiler will
signal a compile-time error."

  Annotation: in the book the makro tries to create an array unnamed[1] or
unnamed[0] - this doesn't have the desired effekt using g++ 4.3.2, thus I
changed the '0' to '-1'
