...
{
  std::cout << "\t\t\tFixedAllocator::Chunk_t::init( std::size_t blockSize, "
               "unsigned char blocks)\n";

  // checks
  assert(blockSize > 0);
  assert(blocks > 0);
  assert((blockSize * blocks) / blockSize ==
         blocks); // overflow check (we are unsigned!)

  // allocate a huge amount of space for data pointer using ::operator new(),
  // this will be splitted in equal blocks to provide it as pool memory
...
{
  std::cout
      << "\t\t\tFixedAllocator::Chunk_t::allocate( std::size_t blockSize)\n";

  if (!blocksAvailable_)
    return 0;

  // overflow check to avoid "unsigned" issues
  if ((firstAvailableBlock_ * blockSize) / blockSize != firstAvailableBlock_)
    return 0;
...
{
  std::cout << "\t\t\tFixedAllocator::Chunk_t::deallocate( void* ptr, "
...
{
  std::cout << "\t\t\tFixedAllocator::Chunk_t* "
               "FixedAllocator::vicinityFind(void* ptr)\n";
  ...
  for (;;) {
    ...
    //*/
    if (lo) {
      // return lo if element starts at lo->pData_ and is within chunkLength
      // size
    ...
    //*/
    if (hi) {
      // return hi if element starts at hi->pData_ and is within chunkLength
      // size
...
/*
  ...
  at a single allocation, only this "blockSize" will be allocated as amount of
blocks, on the other side this also results in a group of equal sized blocks (at
maximum blockSize of size, then ::operator new() will allocate new space)
//*/
FixedAllocator::FixedAllocator(std::size_t blockSize)
    : blockSize_(blockSize), allocChunk_(NULL), deallocChunk_(NULL)
{
  std::cout
      << "\t\t\tFixedAllocator::FixedAllocator(std::size_t blockSize) - ctor\n";
...
FixedAllocator::FixedAllocator(FixedAllocator const &copy)
    : blockSize_(copy.blockSize_), numBlocks_(copy.numBlocks_),
      chunks_(copy.chunks_)
{
  std::cout << "\t\t\tFixedAllocator::FixedAllocator(FixedAllocator const& "
               "copy) - cpy ctor\n";
...
{
  std::cout << "\t\t\tFixedAllocator& FixedAllocator::operator=(const "
               "FixedAllocator& rhs)\n";
...
void *FixedAllocator::allocate()
{
  ...
  if (allocChunk_ == NULL || allocChunk_->blocksAvailable_ == 0) {
    ...
    // remember Chunks == vector< struct Chunk > and chunks_ is and instance of
    // that
    Chunks::iterator iter = chunks_.begin();
